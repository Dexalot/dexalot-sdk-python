...
- **Order data has two shapes that must stay aligned**: `_format_order_data` (contract path, used by `get_order`) and `_transform_order_from_api` (REST API path, used by `get_open_orders`) both produce the same canonical 9-field dict: `internal_order_id`, `client_order_id`, `pair`, `side`, `type`, `price`, `quantity`, `filledQuantity`, `status`. Raw API fields (`id`, `clientordid`, `tx`, `traderaddress`, etc.) are intentionally stripped to prevent MCP/LLM consumers from confusing them with order identifiers.
